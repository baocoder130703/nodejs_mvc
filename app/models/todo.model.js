...
Todo.getAllPublished = result =>{
    sql.query("SELECT * FROM todo published=true",(err,res)=>{
        if(err)
        {
            console.log("err:",err);
            result(null,err);
...
Todo.updateById = (id,todo,result) =>{
    sql.query("UPDATE todo SET title = ? ,description = ?,published = ? WHERE id = ?",
        ...
        (err,res) =>{
            ...
            console.log("update todo :",{id:id,...todo});
...
Todo.removeAll = result=>{
    sql.query("DELETE FROM todo",(err,res)=>{
        ...
        console.log(`delete${res.affectedRows}todo`);
        result(null,res);
    })
}
